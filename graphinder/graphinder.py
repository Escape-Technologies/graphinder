...
@click.option('--file',
              ...
              help='The path to the text file of subdomains to scan')
@click.option('--verbose', '-v', is_flag=True, help='verbose', default=False)
@click.option('--scripts',
              '-s',
              is_flag=True,
              help='check the the scripts on a page for graphql calls',
              default=False)
@click.option(
    '--subdomains',
    '-d',
    is_flag=True,
    help=
    'check for all the subdomains that could be detected for the supplied domain',
    default=False)
@click.option('--subdomains_bruteforce',
              '-b',
              is_flag=True,
              help='detected subdomains via bruteforce (less recomended)',
              default=False)
@click.option('--output_file', '-o', type=click.Path())
def finder(domain: str | None, file: click.File | None, verbose: bool,
           scripts: bool, subdomains: bool, subdomains_bruteforce: bool,
    ...
    if domain is not None and file is not None:
        click.ClickException(
            'Two domain name sources were provided. Only one is needed.')
    ...
    if file is not None:
        handle_domain_file(file, verbose, scripts, subdomains,
                           subdomains_bruteforce, output_file)
        return None
